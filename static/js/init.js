  ...
  $(function(){
    ...
    function checker(){
      if($('input').hasClass("invalid")){
        $('.input-field .helper-text').css("min-height","18px");
        $('.predict').addClass("disabled");

      }
      else{
        $('.predict').removeClass("disabled");
        $('.input-field .helper-text').css("min-height","0px");
      }
    ...
    $('.predict').click(function(){
      ...
      var values = {
       ...
       'RCA_0_1':0, 'RCA_2_3':0,'Female':0, 'Age':0,'Age < 40 years':0,
      ...
      // create a binary feature vector on logic age < 40 years
      
     
      
      values.Age = Number($('#age').val());
      if(values.Age < 40){
        values["Age < 40 years"] = 1;
      }
      ...
      if($('#wall').val()){
        ...
        if($('#wall').val().includes('1')){
          values["Anterior Wall MI"] = 1;
        }
        else if($('#wall').val().includes('2')){
          values["Posterior Wall MI"] = 1;
        }
        else if ($('#wall').val().includes('3')){
          values["Lateral Wall MI"] = 1;
        }
        else if($('#wall').val().includes('4')){
